...
export default function NotificationsPage() {
  ...
  async function handleMarkAllRead() {
    ...
    try {
      ...
      toast.success(t("notif_toast_all_read"));
    } catch {
  ...
  async function handleAccept(id: string) {
    ...
    try {
      ...
      toast.success(t("notif_toast_accepted"));
    } catch {
      toast.error(t("notif_error_failed"));
    }
  }
  ...
  return (
    <div className="flex flex-col">
      <header className="screen-safe-top sticky top-0 z-10 flex items-center justify-between bg-background/80 px-4 py-4 backdrop-blur-md">
        ...
        <h1 className="text-lg font-bold">{t("notif_title")}</h1>
        <Button
          variant="ghost"
        ...
        <div className="space-y-4 px-4 pb-8">
          {unread.map((notification) => {
            ...
            const timeAgo = notification.createdAt?.toDate
              ...
              : "";
            return (
              <Card key={notification.id} className="border-0 bg-secondary/50 shadow-sm">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-1 flex items-center justify-between">
                          ...
                          {notification.type === "milestone" && (
                            <Link href="/records" className="text-xs font-medium text-primary">
                              {t("notif_view_report")}
                            </Link>
                          )}
                          {notification.type === "daily_reminder" && (
                            <Link href="/write" className="text-xs font-medium text-primary">
                              {t("notif_go_record")}
                            </Link>
                          )}
                        </div>
